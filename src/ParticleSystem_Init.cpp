...
void		PositionParticlesRandomly(EngineController *engine, t_ParticleSystemDatas *PSDatas)
{
	...
	glGenVertexArrays(1, &randsuite_vao);
	glGenBuffers(1, &randsuite_vbo);	
	glBindBuffer(GL_ARRAY_BUFFER, randsuite_vbo);
	...
	randsuite_ObjMem = clCreateFromGLBuffer(engine->CLController.Context, CL_MEM_WRITE_ONLY,
		randsuite_vbo, &ret);
	if (ret != CL_SUCCESS)
	{
	...
	engine->CLController.SetKernelArg(0, 3, sizeof(double *), (void *)&randsuite_ObjMem);
